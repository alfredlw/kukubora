...
const symptomes =[
    "écoulement nasal",
    "Enflure sous les yeux et fermeture des yeux",
...
function Step9() {
  ...
  const { q9, addToQ9, removeFromQ9 } = useGlobalContext();
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (checked) {
      addToQ9 && addToQ9(value);
    } else {
      removeFromQ9 && removeFromQ9(value);
    }
  };

  const goTo = () => {
    push("/diagnostic/step10");
  };
      ...
      <main className="max-width-5xl mx-auto">
        <div className="grid md:grid-cols-3 py-8">
          ...
          <div className="col-span-2 pt-20">
            <h1 className="text-4xl">Etape 9</h1>
                ...
                return (
                  <li className="  flex items-start my-2" key={item}>
                    <input
                      type="checkbox"
                      id={`s${index}`}
                      defaultChecked={q9?.includes(item)}
                      value={item}

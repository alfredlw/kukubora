...
const symptomes = [
  ...
  "Tremblement",
  "Cou tordit",
  "Mouvement en forme de cercle",
  "Respiration forte",
  "Fientes verdâtre ou la diarrhée mélangée avec du sang",
  "Mort soudaine",
  "La période d’incubation de trois à cinq jours est suivie de somnolence, de toux, d’éternuement et de halètement",
  "L’accélération de la respiration s’accompagne d’un bruit de dans la gorge",
];

function Step4() {
  const { back, push } = useRouter();

  const { q4, addToQ4, removeFromQ4 } = useGlobalContext();
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (checked) {
      addToQ4 && addToQ4(value);
    } else {
      removeFromQ4 && removeFromQ4(value);
    }
  };

  const goTo = () => {
    push("/diagnostic/step5");
  };
      ...
      <main className="max-width-5xl mx-auto">
        <div className="grid md:grid-cols-3 py-8">
          ...
          <div className="col-span-2 pt-20">
            <h1 className="text-4xl">Etape 4</h1>
                ...
                return (
                  <li className="  flex items-start my-2" key={item}>
                    <input
                      type="checkbox"
                      id={`s${index}`}
                      defaultChecked={q4?.includes(item)}
                      value={item}

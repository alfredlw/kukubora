...
const symptomes = [
  "Pustules sur la caroncule et sur la tête",
  "Mort subite",
  "Forte température et fatigue",
  "La maladie peut paraître toute l’année, mais la plupart du temps pendant les saisons sèches",
];

function Step11() {
  const { back, push } = useRouter();

  const { q11, addToQ11, removeFromQ11 } = useGlobalContext();
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (checked) {
      addToQ11 && addToQ11(value);
    } else {
      removeFromQ11 && removeFromQ11(value);
    }
  };

  const goTo = () => {
    push("/diagnostic/finalize");
  };
      ...
      <main className="max-width-5xl mx-auto">
        <div className="grid md:grid-cols-3 py-8">
          ...
          <div className="col-span-2 pt-20">
            <h1 className="text-4xl">Etape 11</h1>
                ...
                return (
                  <li className="  flex items-start my-2" key={item}>
                    <input
                      type="checkbox"
                      id={`s${index}`}
                      defaultChecked={q11?.includes(item)}
                      value={item}
